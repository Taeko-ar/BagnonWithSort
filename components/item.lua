...
function ItemSlot:UpdateSlotColor()
    if (not self:GetItem()) and self:ColoringBagSlots() then
        if self:IsKeyRingSlot() then
            ...
            if self:GetNormalTexture() then self:GetNormalTexture():SetVertexColor(r, g, b) end
            return
        end
    ...
    if self:GetNormalTexture() then self:GetNormalTexture():SetVertexColor(1, 1, 1) end
end
