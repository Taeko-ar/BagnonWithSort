...
function Frame:PlaceMenuButtons()
	...
	if self:HasSortToggle() then
		local toggle = self:GetSortToggle() or self:CreateSortToggle()
		table.insert(menuButtons, toggle)
	end
...
function SortToggle:New(frameID, parent)
	...
	nt:SetWidth(64 * (20/36))
	nt:SetHeight(64 * (20/36))
	...
	icon:SetAllPoints(b)
	icon:SetTexture([[Interface\Icons\INV_Misc_Broom_01]])
...
function SortToggle:OnClick()
	...
	if E and E:GetModule("Bags") then
		local B = E:GetModule("Bags")
		
		-- Safely initialize ElvUI Bags databases if they are missing (i.e. Bags module is disabled in ElvUI config)
		-- Setting sortInverted to true sorts everything to the bottom
		if not B.db then B.db = { ignoredItems = {}, sortInverted = true } else B.db.sortInverted = true end
		if not E.global then E.global = {} end
		if not E.global.bags then E.global.bags = {} end
		if not E.global.bags.ignoredItems then E.global.bags.ignoredItems = {} end
		if not B.BagFrames then B.BagFrames = {} end

		local groups = self:GetFrameID() == 'bank' and "bank" or "bags"
		local sortFunc = B:CommandDecorator(B.SortBags, groups)
		if sortFunc then
			sortFunc()
		end
	else
		print("ElvUI sorting logic is required, but ElvUI Bags module was not found.")
	end
end
...
function SortToggle:GetSettings()
	return Bagnon.FrameSettings:Get(self:GetFrameID())
end

function Frame:CreateSortToggle()
	local toggle = Bagnon.SortToggle:New(self:GetFrameID(), self)
